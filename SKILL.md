...
host = os.environ.get('PROXMOX_HOST', '192.168.1.100')
port = int(os.environ.get('PROXMOX_PORT', '8006'))
verify_ssl = False  # Use True with proper CA cert for production

if os.environ.get('PROXMOX_TOKEN_ID'):
    # Token-based auth
    proxmox = ProxmoxAPI(
        host, port=port,
        user=os.environ['PROXMOX_TOKEN_ID'],
        password=os.environ['PROXMOX_TOKEN_SECRET'],
...
| Problem | Cause & Fix |
|---------|-------------|
| **401 Unauthorized** | Wrong realm (`@pam` vs `@pve`) — try the other. Or token secret changed when you edited the token in Proxmox UI. |
